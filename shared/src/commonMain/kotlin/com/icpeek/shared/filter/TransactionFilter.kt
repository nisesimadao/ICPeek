package com.icpeek.shared.filter
...
fun TransactionInfo.matchesFilter(filter: TransactionFilter): Boolean {
    ...
    when (filter.filterType) {
        ...
        FilterType.TRANSIT -> {
            if (!transactionType.contains("JR") && !transactionType.contains("運賃")) return false
        }
        FilterType.ALL -> {
            // すべてを許可
        }
    ...
    filter.startDate?.let { start ->
        val transactionDate = kotlinx.datetime.LocalDateTime(
            year = year,
            monthNumber = month,
            dayOfMonth = day,
            hour = 0,
